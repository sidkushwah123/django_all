...
// ----------------------------------LOGO CODE START
  ...
  $image_crop_logo = $('#image_demo_edit_logo').croppie({
...
$(document).ready(function(){
 $('#producer_logo_image').on('change', function(){
   var set_status = true;
    var reader = new FileReader();
         ...
         image.onload = function () {
            ...
            if(height >= 250 && width >= 500)
          {
             $image_crop_logo.croppie('bind', {
              ...
              $('#uploadimageModal_edit_logo').modal('show');
          }
          else
          {
            set_status = false;
  ...
  $('.crop_image_edit_logo').click(function(event){

$('.crop_image_edit_logo').html('Image Uploading');
$('.crop_image_edit_logo').attr("disabled", true);
...
$('#show_logo_image').empty();
$('#producer_logo_image_value').val(response);
$("#show_logo_image").html('<img src="'+response+'"  style="border: 2px solid;" />');
     
$('.crop_image_edit_logo').html('Crop Image');
$('.crop_image_edit_logo').attr("disabled", false);
          $('#uploadimageModal_edit_logo').modal('hide');
